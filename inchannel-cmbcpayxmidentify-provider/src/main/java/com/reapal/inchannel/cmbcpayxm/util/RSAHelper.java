...
public class RSAHelper {
	...
	private static String privateKey = resourceBundle.getString("privateKey");
	private static String publicKey = resourceBundle.getString("publicKey");
	...
	public static void initKey(int keysize) throws Exception {
		try {
			localPrivKey = getPrivateKey(new FileInputStream(privateKey));
			peerPubKey = getPublicKey(new FileInputStream(publicKey));
		} catch (InvalidKeySpecException e) {
			e.printStackTrace();
		}
	}
	...
	public static byte[] encryptRSA(byte[] plainBytes, boolean useBase64Code,
	...
	public static byte[] signRSA(byte[] plainBytes, boolean useBase64Code,
			String charset) throws Exception {
	...
	public static boolean verifyRSA(byte[] plainBytes, byte[] signBytes,
